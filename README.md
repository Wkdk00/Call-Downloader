...
- **REST API** - архитектурный стиль для API
- **WebSocket** (через Nginx) - для реального времени
- **OAuth2** - протокол аутентификации

Приложение представляет собой полноценный микросервисный架构 с фронтендом на Vue.js, бекендом на FastAPI, использованием очередей сообщений, мониторингом и оркестрацией через Docker.
